...
class ResidualConnection(nn.Module):
        ...
        def forward(self, x, sublayer):
            # return self.norm(x + self.dropout(sublayer(x))) used in GPT1
            return x + self.dropout(sublayer(self.norm(x))) # one of the 2 changes suggested in GPT2 paper.
...
class Decoder(nn.Module):
    ...
    def forward(self, x, mask):
        for layer in self.layers:
            x = layer(x, mask)
        # return x   in GPT1
        return self.norm(x) # Another change suggested in the paper.
